...
const DeleteAccount = dynamic(() => import("./components/deleteAccount"), {
  ssr: false,
});
    ...
    const fetchData = async () => {
      try {
        ...
        const appsResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/applications/user/${user._id}`,
          {
            ...
            if (app.jobId) {
              if (!jobsMap[app.jobId]) {
                const jobRes = await fetch(
                  `${process.env.NEXT_PUBLIC_API_URL}/jobs/${app.jobId}`
                );
        ...
        const statsResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/users/${user._id}/stats`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/3">
            ...
            {/* Status Breakdown with open/close toggle */}
            <div className="bg-white shadow rounded-lg p-6">
              <div
                className={`flex justify-between items-center ${
                  showStatusBreakdown ? "mb-4" : ""
                }`}
              >
                ...
                <button
                  onClick={() => setShowStatusBreakdown?.((prev) => !prev)}
                  className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100 transition-colors duration-200 focus:outline-none"
                  aria-label={
                    showStatusBreakdown
                ...
                >
                  <svg
                    className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${
                      showStatusBreakdown ? "rotate-180" : ""
                    }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
